...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    ...
    async loadActive() {
      ...
      const serverMsgs: ChatMessage[] = (r.data?.messages || []).map(m => ({
        ...m,
        streaming: false,
        sources: m.sources || [],
      }))
    ...
    /** 添加一条用户消息到当前对话 */
    pushUserMessage(content: string): ChatMessage {
    ...
    pushAssistantPlaceholder(): ChatMessage {
      const msg: ChatMessage = {
        id: Date.now() + 1,
        role: 'assistant',
        content: '',
