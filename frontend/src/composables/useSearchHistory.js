 ...
 * @param {number|string|null} uid - 用户 ID
...
/**
 * 从 localStorage 加载当前用户的历史记录到共享状态
 * 解析失败时回退为空数组
 */
function loadInto() {
  try {
    searchHistory.value = JSON.parse(localStorage.getItem(historyKey(loadedFor)) || '[]')
  } catch {
    searchHistory.value = []
  }
}
 ...
 * @returns {{ searchHistory: import('vue').Ref<string[]>, addToHistory: (kw: string) => void, removeHistory: (kw: string) => Promise<boolean> }}
 */
export function useSearchHistory() {
   ...
   */
  async function removeHistory(kw) {
    try {
